...
const ALLOWED_ORIGINS = new Set([
  ...
  // ქვედომენები თუ დაგჭირდება, დაამატე აქ
])

app.use(
  '*',
  cors({
    origin: (origin) => (origin && ALLOWED_ORIGINS.has(origin) ? origin : 'https://www.sansotec.ge'),
...
app.post('/api/auth/login', async (c) => {
  try {
    ...
    const userRow = await c.env.DB
      .prepare(
        `SELECT id, username, password_hash, role, customer_id
         FROM appusers WHERE username = ?`,
      )
      .bind(username)
      .first<{ id: number; username: string; password_hash: string; role: string; customer_id: string | number | null }>()
...
app.get('/api/auth/me', (c) => c.json({ ok: false, role: 'guest' }, 401))
...
app.post('/api/customers', async (c) => {
  try {
    ...
    await c.env.DB.prepare(
      ...
      .bind(id, name, ident_code, salon_name, address, phone, contact_person, device, device_sn)
      .run()
...
app.put('/api/customers/:id', async (c) => {
  ...
  const { name, ident_code, salon_name = '', address = '', phone = '', contact_person = '', device = '', device_sn = '' } = body
  ...
  const dup = await c.env.DB.prepare('SELECT id FROM customers WHERE ident_code = ? AND id <> ?')
    .bind(ident_code, id)
    .first()
  ...
  await c.env.DB.prepare(
    ...
    .bind(name, ident_code, salon_name, address, phone, contact_person, device, device_sn, id)
    .run()
...
app.post('/api/customers/:id/services', async (c) => {
  try {
    ...
    const { date_ddmmyyyy = '', purpose = '', price_gel = 0, technician = '' } = body
    ...
    await c.env.DB.prepare(
      ...
      .bind(uuid, id, iso, String(purpose), Number(price_gel || 0), String(technician || ''))
      .run()
...
app.put('/api/customers/:id/services/:sid', async (c) => {
  try {
    ...
    const ex = await c.env.DB.prepare(`SELECT id FROM service_records WHERE id=? AND customer_id=?`).bind(sid, id).first()
    ...
    await c.env.DB.prepare(
      ...
      .bind(iso, String(purpose), Number(price_gel || 0), String(technician || ''), sid, id)
      .run()
...
app.post('/api/customers/:cid/services/:sid/files', async (c) => {
  try {
    ...
    const ex = await c.env.DB.prepare(`SELECT id FROM service_records WHERE id = ?`).bind(sid).first()
    ...
    await c.env.DB.prepare(
      ...
      .bind(fid, sid, kind, originalName, r2Key, arrayBuffer.byteLength)
      .run()
...
// ფაილის ჩამოტვირთვა/ჩვენება
app.get('/api/files/:fid', async (c) => {
  try {
    const fid = c.req.param('fid')
    if (!fid) return c.json({ ok: false, message: 'არასწორი file ID' }, 400)

    const row = await c.env.DB.prepare(`SELECT r2_key, filename FROM service_files WHERE id = ?`).bind(fid).first<{
      r2_key: string
      filename: string
    }>()
...
app.delete('/api/files/:fid', async (c) => {
  try {
    ...
    const row = await c.env.DB.prepare(`SELECT r2_key FROM service_files WHERE id = ?`).bind(fid).first<{ r2_key: string }>()
    if (!row) return c.json({ ok: false, message: 'ფაილი ვერ მოიძებნა' }, 404)
...
// ─────────────────────────────────────────────────────────────
// APPUSERS — List/Create/Update/Delete  (პაროლი ჰეშდება)
app.get('/api/appusers', async (c) => {
  try {
    const { results } = await c.env.DB
      .prepare(`SELECT id, username, role, customer_id, created_at FROM appusers ORDER BY created_at DESC`)
      .all()
    return c.json({ ok: true, items: results || [] })
  } catch (e: any) {
    return c.json({ ok: false, message: e?.message || 'DB error' }, 500)
...
app.post('/api/appusers', async (c) => {
  try {
    ...
    const { username = '', password = '', role = '', customer_id = null } = body

    if (!username) return c.json({ ok: false, message: 'username სავალდებულოა' }, 400)
    if (!password) return c.json({ ok: false, message: 'პაროლი სავალდებულოა' }, 400)

    const allowed = ['admin', 'customer', 'technician', 'viewer']
    if (!allowed.includes(String(role))) return c.json({ ok: false, message: 'არასწორი როლი' }, 400)

    if (role === 'customer' && !/^\d+$/.test(String(customer_id ?? '')))
      return c.json({ ok: false, message: 'customer_id უნდა იყოს მხოლოდ ციფრები' }, 400)

    const dup = await c.env.DB.prepare(`SELECT 1 FROM appusers WHERE username=?`).bind(username).first()
    if (dup) return c.json({ ok: false, message: 'ეს username უკვე არსებობს' }, 409)
    ...
    await c.env.DB.prepare(
      ...
      .bind(username, hash, role, role === 'customer' ? Number(customer_id) : null)
      .run()
...
app.put('/api/appusers/:id', async (c) => {
  try {
    const id = c.req.param('id')
    if (!id) return c.json({ ok: false, message: 'არასწორი ID' }, 400)
    ...
    const ex = await c.env.DB.prepare(`SELECT id FROM appusers WHERE id=?`).bind(id).first()
    if (!ex) return c.json({ ok: false, message: 'მომხმარებელი ვერ მოიძებნა' }, 404)

    const dup = await c.env.DB.prepare(`SELECT id FROM appusers WHERE username=? AND id<>?`)
      .bind(username, id)
      .first()
    if (dup) return c.json({ ok: false, message: 'ეს username უკვე გამოყენებულია' }, 409)

    if (password) {
      const hash = hashSync(String(password), 10)
      await c.env.DB.prepare(
        `UPDATE appusers
         SET username=?, password_hash=?, role=?, customer_id=?, updated_at=datetime('now')
         WHERE id=?`,
      )
        .bind(username, hash, role, role === 'customer' ? Number(customer_id) : null, id)
        .run()
    } else {
      await c.env.DB.prepare(
        `UPDATE appusers
         SET username=?, role=?, customer_id=?, updated_at=datetime('now')
         WHERE id=?`,
      )
        .bind(username, role, role === 'customer' ? Number(customer_id) : null, id)
        .run()
    }
